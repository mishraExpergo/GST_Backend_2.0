...
@Injectable()
export class GstDashboardFilingBehaviourService {
  constructor(
    @Optional()
  ...
  async getFilingBehaviour(params: {
    loanId?: string;
    pan?: string;
    asOf?: Date;
  }) {
    if (!this.gstrTrackModel) {
      throw new ServiceUnavailableException(
        'MongoDB is not enabled. Set ENABLE_MONGO=true to load filing behaviour data.',
      );
    }
    ...
    // Cover 5y window FYs (track docs keyed by financialYear string)
    ...
    // Filter to FYs overlapping 5y window (keep extras out of facts via date filter later)
    ...
    const inWindowDocs = trackDocs.filter((doc) => {
      ...
      if (!fy) return true; // still extract periods; month filter applies later
      // Normalize "FY 2021-22" vs "FY2021-22"
    ...
    return {
      ...
      ranges: {
        '1y': buildFilingBehaviourRangeBlock(facts, 1, asOf),
        '3y': buildFilingBehaviourRangeBlock(facts, 3, asOf),
        '5y': buildFilingBehaviourRangeBlock(facts, 5, asOf),
      },
    };
  }
  ...
  > {
    ...
    // Always include GSTINs that already have track docs

...
import { buildNonFilingRangeBlock } from './gst-dashboard-non-filing-behaviour.util';

@Injectable()
export class GstDashboardNonFilingBehaviourService {
  constructor(
    @Optional()
  ...
  async getNonFilingBehaviour(params: {
    loanId?: string;
    pan?: string;
    asOf?: Date;
  }) {
    if (!this.gstrTrackModel) {
      throw new ServiceUnavailableException(
        'MongoDB is not enabled. Set ENABLE_MONGO=true to load non-filing behaviour data.',
      );
    }
    ...
    return {
      ...
      ranges: {
        '1y': buildNonFilingRangeBlock(facts, 1, asOf),
        '3y': buildNonFilingRangeBlock(facts, 3, asOf),
        '5y': buildNonFilingRangeBlock(facts, 5, asOf),
      },
    };
  }
